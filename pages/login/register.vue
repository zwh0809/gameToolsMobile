<template>
	<view class="register">
		...
		<view class="content">
			...
			<!-- 主体 -->
			<view class="main">
				<wInput v-model="LoginForm.username" type="text" placeholder="登录账号"></wInput>
				<wInput v-model="LoginForm.nickname" type="text" placeholder="用户昵称"></wInput>
				<wInput v-model="LoginForm.email" type="text" placeholder="邮箱地址"></wInput>
				<wInput v-model="LoginForm.password" type="password" placeholder="输入密码" isShowPass></wInput>
				<wInput v-model="LoginForm.password2" type="password" placeholder="确认密码" isShowPass></wInput>
			</view>
...
<script>
	...
	import {
		onlyCnCheck
	} from '../../common/js/verification.js' // 验证函数
	export default {
		data() {
			return {
				//logo图片 base64
		...
		methods: {
			startReg() {
				...
				let cnCheck = onlyCnCheck(nickname);
				if (!cnCheck) {
					uni.showToast({
						icon: 'none',
						title: '用户昵称只能输入中文',
				...
				that.$request.post(root.loginUrl + '/register', postData, header).then(res => {
					if(res==="Success"){
						...
						uni.showModal({
						    ...
						    success: function (res) {
						        ...
						        if (res.confirm) {
									...
									that.$request.post(root.loginUrl + '/login', loginpostData, header).then(res => {
										console.log(res);
										if (res === 'Fail') {
											uni.showToast({
												icon: 'none',
