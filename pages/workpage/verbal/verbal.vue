<template>
	<view>
		<view class="box"><QSTabs :current="current_1" :tabs="tabs_1" :width="250" Number="100" backgroundColor="#fff" @change="change_tap($event, '1')" /></view>
		<view class="" v-show="current_1 === 0">
			<button  class="margin-top widthnine" @click="puah_questions()">添加通用问答</button>
			<button  class="margin-top widthnine" @click="submit_questions()">提交</button>
			<view class="">
		...
		<view class="" v-show="current_1 === 1" >
			<button  class="margin-top widthnine" @click="goto_editor_goods(empty, pushdata)">添加角色</button>
			<!-- <view class="one_intention"   v-for="(item, index) of intention">
				<QSInput variableName="input" required layout="row" title="回答" v-model=""></QSInput>
				<QSInput variableName="input" required layout="row" title="回答" v-model="item.value"></QSInput>
			</view> -->
		</view>
		<view class="" v-show="current_1 === 2">
			<button  class="margin-top widthnine" @click="goto_editor_goods(empty, pushdata)">添加角色</button>
			<view class="">
				3
			</view>
...
import speechcraft from '@/common/speechcraft.json';
export default {
	components: {
		QSTabs
	},
	...
	methods: {
		...
		submit_questions() {
			...
			that.$request
				.put('/generalQA/' + that.workinfo.id + '/', postData)
				.then(res => {
					console.log(res);
					if (res.code === 1) {
						uni.showModal({
		...
		getquestionsList() {	
			...
			let that = this;
			that.$request
				.get('/generalQA/'+that.workinfo.id+'/')
				.then(res => {
